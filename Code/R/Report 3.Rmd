...
Firstly,the density of Gamma is:
...
The distribution of (SAR) Gamma is:

$$Z \sim {} \Gamma(\mu, L)$$
the density of (SAR) Gamma is:

$$
f_\Gamma(z; L, \mu)=\frac{L^L}{\mu^L\Gamma(L)}z^{L-1}exp{(\frac{-Lz}{\mu})}
$$
When L = 1,the density of (SAR) Gamma is:
...
At this time,moment estimation for gamma is:
...
There are two ways to solve  α and $\gamma$, by E(z) and E(z^2), or by E(z) and D(z).

## E(z) and E(z^2):
This is the first solution by E(z) and E(z^2).
When m1 = E(z), m2 = E(z^2)  
The parameters:
$$
...
##(E(z), D(z))
This is another solution by E(z) and D(z).
When m1=E(z), m2=D(z), m3 = m2/m1^2  
The parameters:
$$
